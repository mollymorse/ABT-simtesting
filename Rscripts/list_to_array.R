...
this_list <- east_3D_trans_all  # CHANGE HERE: which list we want to transform
...
RW_juv <- rep(NA, dim(west_juv)[4])
RW_ad  <- rep(NA, dim(west_ad)[4])
RE_all <- rep(NA, dim(east_all)[4])
...
#west juvenile
boxplot(RW_juv)
quant_tar <- quantile(RW_juv, 0.75)
target <- which(abs(RW_juv - quant_tar) == min(abs(RW_juv - quant_tar))) #IDs the realization of the 75th percentile
RW_juv[which(abs(RW_juv - quant_tar) == min(abs(RW_juv - quant_tar)))] #IDs the R value of the 75th percentile
west_juv_target <- west_juv[ , , , target]  #saves realization of the 75th percentile

#west adult
boxplot(RW_ad)
quant_tar <- quantile(RW_ad, 0.75)
target <- which(abs(RW_ad - quant_tar) == min(abs(RW_ad - quant_tar))) #IDs the realization of the 75th percentile
RW_ad[which(abs(RW_ad - quant_tar) == min(abs(RW_ad - quant_tar)))] #IDs the R value of the 75th percentile
west_ad_target <- west_ad[ , , , target]  #saves realization of the 75th percentile

#east all
boxplot(RE_all)
quant_tar <- quantile(RE_all, 0.75)
target <- which(abs(RE_all - quant_tar) == min(abs(RE_all - quant_tar))) #IDs the realization of the 75th percentile
RE_all[which(abs(RE_all - quant_tar) == min(abs(RE_all - quant_tar)))] #IDs the R value of the 75th percentile
east_all_target <- east_all[ , , , target]  #saves realization of the 75th percentile



#### New OM movement matrices ####
...
west_target <- abind(west_juv_target_2, west_ad_target_2, along = 4)



# East: all ages same
east_target <- array(rep(east_all_target, 29), dim = c(7, 7, 4, 29), 
...
new_move <- abind(east_target, west_target, along = 5)  
